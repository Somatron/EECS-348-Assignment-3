...
int main() { //when we run the file
    std::string filename; //create filename variable

    std::cout << "Enter your filename: "; //ask user to type out a file
    std::getline(std::cin, filename); //use getline in case file path contains spaces

    std::ifstream inputFile(filename); //inherit the file
    
    inputFile.is_open(); //open the file
    
    std::cout << "\n------- OUTPUT -------\n"; //new line since we enter file

    MaxHeap inbox; //overall heap will be represented as an inbox
    std::string line; //extract line by string

    while (std::getline(inputFile, line)) { //loop through line
